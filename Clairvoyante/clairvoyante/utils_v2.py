...
#     print("Inside GetTrainingArray")
    ...
    # print(f"After processing bed_fn, tree length:{len(tree)}")
    # print(f"Tree keys: {tree.keys()}")
    # print(f"Len chr21: {len(tree['chr21'])}")
    # print(f"Len chr22: {len(tree['chr22'])}")
    ...
    # print(f"After processing var_fn, tree length:{len(Y)}")
    # print(f"Tree keys: {Y.keys()}")
    # print(f"Len chr21: {len(tree['chr21'])}")
    # print(f"Len chr22: {len(tree['chr22'])}")
    ...
    for row in f.stdout:
        # print("Inside X for loop")
        row = row.decode()
        chrom, coord, seq, mat = UnpackATensorRecord(*(row.split()))
        if bed_fn != None:
            if chrom not in tree:
                # print("exiting because chrom not in tree")
                continue
            if len(tree[chrom].at(int(coord))) == 0: 
                # print("exiting because chrom not in tree, 2")
                continue
        seq = seq.upper()
#         print(seq)
        if seq[param.flankingBaseNum] not in ["A","C","G","T"]:
            # print("Exiting because seq[param.flankingBaseNum] not in [A,C,G,T]")
            # print(seq[param.flankingBaseNum])
            continue
        key = chrom + ":" + coord
        ...
        total += 1
        if total % 10000 == 0: 
            print("Processed %d tensors" % total)
    print("Processed %d tensors" % total)    
    f.stdout.close()
    f.wait()
    
    # print(f"After processing var_fn, tree length:{len(X)}")
    # print(f"Tree keys: {X.keys()}")
    # print(f"Len chr21: {len(tree['chr21'])}")
    # print(f"Len chr22: {len(tree['chr22'])}")

    allPos = sorted(list(X.keys()))
    if shuffle == True:
    ...
    count = 0
    total = 0
    print(f"Stuff in allPos: {len(allPos)}")
