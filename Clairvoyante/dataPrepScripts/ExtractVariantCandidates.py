...
def MakeCandidates( args ):
    ...
    if os.path.isfile("%s.fai" % (args.ref_fn)) == False:
        print("Fasta index %s.fai doesn't exist." % (args.ref_fn))
        sys.exit(1)
    ...
    if p1.returncode != 0 or len(refSeq) == 0:
        print("Failed to load reference seqeunce.")
        sys.exit(1)
    ...
    if args.bed_fn != None:
        ...
        if args.ctgName not in tree:
            print("ctgName is not in the bed file, are you using the correct bed file (%s)?" % (args.bed_fn))
            sys.exit(1)
    ...
    for l in p2.stdout:
        ...
        if RNAME != args.ctgName:
            print("Exiting because RNAME != args.ctgName")
            print(RNAME, args.ctgName)
            continue
        ...
        if MQ < args.minMQ:
            print("Exiting because MQ < args.minMQ")
            print(MQ, args.minMQ)
            continue
        skipBase = 0
        totalAlnPos = 0
        ...
        if 1.0 - float(skipBase) / (totalAlnPos + 1) < 0.55:
            # print("Exiting because 1.0 - float(skipBase) / (totalAlnPos + 1) < 0.55")
            # print(1.0 - float(skipBase) / (totalAlnPos + 1), 0.55)# skip a read less than 55% aligned
            continue
        ...
        for m in re.finditer(cigarRe, CIGAR):
            ...
            if m.group(2) in ("M", "=", "X"):
                ...
                for pos, base in matches:
                    ...
                    # if pos not in pileup.keys():
                    #     print(f"Adding key: {pos} to pileup")
                    #     pileup[pos] = {base:0}
                    # print(pileup.keys(), pos, base)
                    # print(pileup[pos].keys(), base, )
                    pileup[pos][chr(base)] += 1
                del matches
            elif m.group(2) == "I":
        ...
        while sweep < POS:
            ...
            if out != None:
                ...
                can_fp.stdin.write(bytes(outline, encoding="utf-8"))
                can_fp.stdin.write(bytes("\n", encoding="utf-8"))
            del pileup[sweep]
            sweep += 1
    ...
    remainder.sort()
    # print(f"Remainder, sorted: {remainder}")
    ...
    if processedReads == 0:
        print("No read has been process, either the genome region you specified has no read cover, or please check the correctness of your BAM input (%s)." % (args.bam_fn))
        sys.exit(0)
    else:
        print(f"Number of reads processed:{processedReads}")
...
def main():
    ...
    print(f"Inside ExtractVariantCandidates: {args}")
    MakeCandidates(args)
