...
def OutputVariant( args ):
    ...
    for row in vcf_fp.stdout:
        ...
        if row[0][0] == "#":
            continue
        if row[0] != ctgName:
            print("Exiting because row[0] != ctgName")
            print(row[0], ctgName)
            continue
        if ctgStart != None and ctgEnd != None:
        ...
        if p1 == 1 and p2 == 2 and row[4].find(",") != -1:
            ...
            for i in gts:
                if len(i) < shortestLen:
                    ...
                    shortestGT = i
            row[4] = shortestGT
        var_fp.stdin.write(bytes(" ".join([row[0], row[1], row[3], row[4], str(p1), str(p2)]), encoding="utf-8"))
        var_fp.stdin.write(bytes("\n", encoding="utf-8"))
    vcf_fp.stdout.close()
    vcf_fp.wait()
